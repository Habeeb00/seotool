        ...
        <div className={`verdict-card ${verdictClass} animate-scale-in`} id="verdict-card">
          ...
          {result.difference > 0 && (
            <div className="verdict-highlight">
              You're paying <strong>{formatPoints(result.difference)}</strong> points more than average
              ({Math.abs(result.percentage)}% higher)
            </div>
          )}
          {result.difference < 0 && (
            <div className="verdict-highlight verdict-highlight-good">
              You're saving <strong>{formatPoints(Math.abs(result.difference))}</strong> points compared to average
              ({Math.abs(result.percentage)}% lower)
            </div>
          )}
        </div>
          ...
          <div className="alternatives-section animate-fade-in-up" id="alternatives-section">
            <div className="alternatives-header">
              <div className="alternatives-badge">
                🔥 We found {alternatives.length} better ways to book this flight
              </div>
              <h3 className="alternatives-title">Better Options Available</h3>
              <p className="alternatives-subtitle">
                Unlock these cheaper alternatives on Flightpoints
              </p>
            </div>
            ...
            <div className="alternatives-grid">
              {alternatives.map((alt, i) => (
                <div key={i} className={`alternative-card ${i === 0 ? 'alt-best' : ''} animate-fade-in-up delay-${i + 1}`}>
                  <div className="alt-blur-overlay"></div>
                  <div className="alt-content">
                    ...
                    <div className="alt-points">
                      ...
                      <span className="alt-points-label">points</span>
                    </div>
                    <div className="alt-details">
                      <span className="alt-program">{alt.program}</span>
                      <span className="alt-timing">{alt.timing}</span>
                    </div>
                    <div className="alt-savings">
                      Save {formatPoints(alt.savings)} pts
                    </div>
                  </div>
                  <div className="alt-lock">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
                      <rect x="3" y="11" width="18" height="11" rx="2" stroke="currentColor" strokeWidth="2"/>
                      <path d="M7 11V7C7 4.239 9.239 2 12 2C14.761 2 17 4.239 17 7V11" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                    </svg>
                  </div>
                </div>
              ))}
